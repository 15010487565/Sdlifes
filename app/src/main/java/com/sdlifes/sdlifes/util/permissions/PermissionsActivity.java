package com.sdlifes.sdlifes.util.permissions;
...
//                    Toast.makeText(this, getString(R.string.vantop_isemulator), Toast.LENGTH_SHORT).show();
    ...
    protected boolean hasAllPermissionsGranted(String[] permissions, @NonNull int[] grantResults, int requestCode) {
        for (int i = 0; i < permissions.length; i++) {
            ...
            Log.e("TAG_权限","permission="+permission+";grantResult="+grantResult);
            if (grantResult == PackageManager.PERMISSION_DENIED) {//拒绝
                try {
                    ...
                    Log.e("TAG_权限","permission="+permission+";isShow="+isShow);
                    if (!isShow) {//勾选拒绝再次提示
                        switch (requestCode) {
